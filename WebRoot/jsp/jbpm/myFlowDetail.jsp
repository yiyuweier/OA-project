...
<body>
	...
	<table cellpadding="0" cellspacing="0" 
			...
			<s:iterator value="#session.mydocumentlist">
				<s:if test="%{sendto==null}">
				 	<tr>
						...
						<td style="border-bottom:1px solid #BBD2FF;border-right:1px solid #BBD2FF;padding: 5px;font-size:13px;text-align:center;">${D_content}</td>
						<td style="border-bottom:1px solid #BBD2FF;border-right:1px solid #BBD2FF;padding: 5px;font-size:13px;text-align:center;">${type==1?'串行':'并行'}</td>
						<td style="border-bottom:1px solid #BBD2FF;padding: 5px;font-size:13px;text-align:center;">默认状态下处理人均未处理，下边列出的均为已处理的</td>
					</tr> 
				</s:if>
				<s:else>
					<tr>
		...
		<s:if test="#session.flowtype=='AskForLeave'">
			<s:iterator value="#session.mydocumentlist">
			 	<tr>
					<td style="border-bottom:1px solid #BBD2FF;border-right:1px solid #BBD2FF;padding: 5px;font-size:13px;">${D_name}</td>
					<td style="border-bottom:1px solid #BBD2FF;border-right:1px solid #BBD2FF;padding: 5px;font-size:13px;text-align:center;">${sendtoName}</td>
					<td style="border-bottom:1px solid #BBD2FF;border-right:1px solid #BBD2FF;padding: 5px;font-size:13px;text-align:center;">请假</td>
					<td style="border-bottom:1px solid #BBD2FF;padding: 5px;font-size:13px;text-align:center;">${ispass==0?'不同意-':(ispass==1?'通过':'没处理')}</td>
				</tr> 
			</s:iterator>
		</s:if>
		<s:elseif test="#session.flowtype=='Message'">
			<s:iterator value="#session.mydocumentlist">
			 	<tr>
					<td style="border-bottom:1px solid #BBD2FF;border-right:1px solid #BBD2FF;padding: 5px;font-size:13px;">${D_name}</td>
					<td style="border-bottom:1px solid #BBD2FF;border-right:1px solid #BBD2FF;padding: 5px;font-size:13px;text-align:center;">${sendtoName}</td>
					<td style="border-bottom:1px solid #BBD2FF;border-right:1px solid #BBD2FF;padding: 5px;font-size:13px;text-align:center;">通知公告</td>
					<td style="border-bottom:1px solid #BBD2FF;padding: 5px;font-size:13px;text-align:center;">${state=='false'?'未读':'已读'}</td>
				</tr> 
			</s:iterator>
		</s:elseif>
