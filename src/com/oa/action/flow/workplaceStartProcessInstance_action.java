...
public class workplaceStartProcessInstance_action {
	...
	public String askForLeave(){//请假流程的实例化
		...
		String processuserid = participantsArray[0];//获取发送到的用户的id//-----
...
//			identityService.createMembership(u.getId().toString(), "officeGroups");//将该用户添加到组里
		...
		if(D_type.equals("2")){//并行审批
			...
			//******************************************************************************************
			//发送推送消息
			System.out.println("发送推送消息，推送消息发送给="+allparticipants);
			Comet4j comet4j = new Comet4j();
...
//			String SerialPeople = participantsArray[0];//获取第一个审批的人
	...
	public String DocumentForeditor(){//审批没有通过时，可以重新发起新的流程，此时要先将这个流程之前的数据删除
...
//		List<String> participants = new ArrayList<String>();
//		String[] participantsArray = D_sendto.split(",");//获取所有的审批人
...
//		}
		...
		List<Document> mydocumentlist = documentdao.getDocumentByInitiatorAndDocumentName(beginuser, D_name);//获取之前发起的流程的所有的信息（数据库中对应的记录）
		for(Document document : mydocumentlist){//逐条删除这些记录
